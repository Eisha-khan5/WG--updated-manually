...
export const getPersonalizedRecommendations = async (
  ...
  try {
    ...
    if (!allProducts || allProducts.length === 0) {
      console.log('No products found in database for personalized recommendations');
      return [];
    }
    ...
    if (preferences.fabrics && preferences.fabrics.length > 0) {
    ...
    if (!data || data.length === 0) {
      // If no personalized recommendations, get fallback
      return await getFallbackRecommendations(limit);
    }
...
const getFallbackRecommendations = async (limit: number = 6): Promise<ProductProps[]> => {
  try {
